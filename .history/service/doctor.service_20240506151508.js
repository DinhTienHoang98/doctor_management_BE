...
function sendOtp() {
    ...
    fetch("https://5yk1gj.api.infobip.com/sms/2/text/advanced", requestOptions)
        .then((result) => {
            ...
            return db.Account.findOne({ where: { email: req.body.email } });
        })
        .then((existingAccount) => {
            if (existingAccount) {
                // Nếu tài khoản đã tồn tại, cập nhật OTP cho tài khoản này
                return existingAccount.update({ otp_code: otp });
            } else {
                // Nếu không, tạo một tài khoản mới với OTP
                return db.Account.create({ otp_code: otp });
            }
        })
        .then((updatedOrCreatedAccount) => {
            console.log("OTP saved to database:", updatedOrCreatedAccount); // Log thông tin về OTP đã được lưu
        })
...
async function confirmOtp(email, res) {
    try {
        const xxx = await getEmail();
        const account = xxx.findOne(u => u.email === email);
        ...
        // So sánh OTP
        if (account.otp_code !== req.body.otp) {
