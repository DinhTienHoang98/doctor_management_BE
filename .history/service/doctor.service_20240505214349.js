...
function sendOtp() {
    ...
    const otp = genOtp(6);
    // TODO: Save OTP vao bang acc
    // Lưu OTP vào bảng accounts
    const queryString = "INSERT INTO accounts (otp) VALUES (?)";
    connection.query(queryString, [otp], function(error, results, fields) {
        if (error) {
            console.error("Lỗi khi lưu OTP:", error);
        } else {
            console.log("OTP đã được lưu thành công.");
        }
};
