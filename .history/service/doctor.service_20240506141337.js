...
function sendOtp() {
    ...
    fetch("https://5yk1gj.api.infobip.com/sms/2/text/advanced", requestOptions)
        ...
        .then((createdAccount) => {
            console.log("OTP saved to database:", createdAccount); // Log thông tin về OTP đã được lưu
        })
...
async function confirmOtp(req, res) {
    try {
        // Lấy OTP từ bảng accounts dựa trên gmail
        const account = await db.Account.findOne({ where: { gmail: req.body.gmail } });

        // Kiểm tra xem có tìm thấy tài khoản dựa trên gmail hay không
        if (!account) {
            return res.status(400).json({ message: "Gmail không tồn tại" });
        }
