...
async function changePassword(email, password, res) {
    try {
...
async function authenticate({ username, password }) {
    ...
    const user = account.find(u => u.username === username && u.password === password);
    if (user) {
...
FROM
    ...
    services se ON ds2.service_id = se.id
        LEFT JOIN
    accounts a ON d.id = a.doctor_id
GROUP BY d.id, d.fullname, d.workplace, d.birth_year, d.introduce, d.training_process, d.gender, d.phone_number;
